...
def plot_pitch(
    ...
    for s in signs: # plots each line seperately
        ...
        if attacking_third:
            attacking_third_length = field_dimen[0]/3.
            ax.plot(
                [s*half_pitch_length-s*attacking_third_length,s*half_pitch_length-s*attacking_third_length],
    ...
    if grids:
        ...
        for y in ygrid:
            ax.plot([-half_pitch_length,half_pitch_length],[y,y],'grey',linewidth=linewidth/2.)
        for x in xgrid:
            ax.plot([x,x],[-half_pitch_width,half_pitch_width],'grey',linewidth=linewidth/2.)
...
def plot_frame(
    event: pd.Series,
    actor_team_players: list,
    opponent_team_players: list,
    figax=None,
    team_colors=('r','b'),
    field_dimen=(105.0,68.0), 
    ...
    if figax is None: # create new pitch 
        fig,ax = plot_pitch(field_dimen=field_dimen,attacking_third=True,)
    else: # overlay on a previously generated pitch
        fig,ax = figax # unpack tuple

    # plot home & away teams in order
    for ap in actor_team_players:
        color = team_colors[0]
        ax.plot(ap.position[0], ap.position[1], color+'o', markersize=Playermarkersize, alpha=PlayerAlpha)
        if include_player_velocities:
            ax.quiver(ap.position[0], ap.position[1], ap.velocity[0], ap.velocity[1], color=color, scale_units='inches', scale=10., width=0.0015, headlength=5, headwidth=3, alpha=PlayerAlpha)
    for op in opponent_team_players:
        color = team_colors[1]
        ax.plot(op.position[0], op.position[1], color+'o', markersize=Playermarkersize, alpha=PlayerAlpha)
        if include_player_velocities:
            ax.quiver(op.position[0], op.position[1], op.velocity[0], op.velocity[1], color=color, scale_units='inches', scale=10., width=0.0015, headlength=5, headwidth=3, alpha=PlayerAlpha)

    # plot ball
    ax.plot(event['Start X'], event['Start Y'], 'ko', markersize=6, alpha=1.0, linewidth=0)
...
def plot_events(
    ...
    if 'Marker' in indicators:
        ax.plot(event['Start X'], event['Start Y'], color+marker_style, alpha=alpha )
    if 'Arrow' in indicators:
        ax.annotate("", xy=event[['End X','End Y']], xytext=event[['Start X','Start Y']], alpha=alpha, arrowprops=dict(alpha=alpha,width=0.5,headlength=4.0,headwidth=4.0,color=color),annotation_clip=False)
    if annotate:
        textstring = event['Type'] + ': ' + event['From']
        ax.text(event['Start X'], event['Start Y'], textstring, fontsize=10, color=color)
...
def plot_pitchcontrol_for_event(
    ...
    # pass_frame = events.loc[event_id]['Start Time [s]']
